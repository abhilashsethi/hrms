				...
				{projectData?.clientId ? (
					<>
						...
						{projectData?.clientDocs?.length ? (
							<div className="py-4 grid lg:grid-cols-3 gap-3 flex-wrap justify-center items-center">
								{projectData?.clientDocs?.map((item: any) => (
									<div className="relative mt-3 rounded-md shadow-jubilation">
										{user?.role?.name === "CEO" ||
										user?.role?.name === "HR" ||
										user?.role?.name === "DIRECTOR" ||
										user?.role?.name === "COO" ||
										...
										<Link href={item?.link} key={item?.id} className="">
											<div className="grid pt-2 gap-2 justify-items-center">
												<Link href={item?.link}>
													<img
