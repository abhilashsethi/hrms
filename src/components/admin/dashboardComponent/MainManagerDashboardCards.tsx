...
const MainManagerDashboardCards = ({ data }: Props) => {
  const { user } = useAuth();
  ...
  const cards = [
    ...
    },
    {
      ...
      count: dashboardData?.assignAssetCount || 0,
      link: "/admin/assets/all-assets",
    },
  ];
  const Quick_Access = [
              ...
              return (
                <Tooltip title={item?.title} key={index}>
                  <Link
                    ...
                    className="w-24 h-24 rounded-full group flex justify-start items-center hover:scale-105 ease-in-out transition-all duration-400 cursor-pointer !text-blue-600 flex-col"
                  >
                    <div
...
export default MainManagerDashboardCards;
