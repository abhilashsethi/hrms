...
import { SearchLoaderAnime } from "components/core";
...
const RoleWisePageAccess = ({ open, onClose, roleId }: Props) => {
	...
	const filteredPageList = PageList.filter((item) =>
		item.name.toLowerCase().includes(isSearchTerm.toLowerCase())
	);
	const hasSearchResults = filteredPageList.length > 0;
...
const MoreOption = ({ item, roleId }: any) => {
	...
	const handleDelete = async (data: any, roleData: Role) => {
		Swal.fire({
			title: "Are you sure?",
...
const PageList = [
	...
	{
		id: 80,
		value: "/admin/security/all-shifts",
		icon: <ICONS.Shift_Configure />,
