...
const ChangeClientProfile = ({ open, handleClose, mutate }: Props) => {
  const [loading, setLoading] = useState(false);
  const router = useRouter();
  const { data: clientData } = useFetch<Client>(`clients/${router?.query?.id}`);
          ...
          >
            ...
            >
              {formik?.values?.photo ? (
                ...
                <img
                  className="h-full w-full object-cover"
                  src={clientData?.photo || DEFAULTPROFILE.src}
                  alt="Preview"
                />
              )}
            </div>
...
export default ChangeClientProfile;
