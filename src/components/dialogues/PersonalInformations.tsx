...
const PersonalInformations = ({
  ...
  const validationSchema = Yup.object().shape({
    panNo: Yup.string().matches(
      /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/,
      "Invalid PAN number. Use capital letters and numbers."
    ),
    ...
    uanNo: Yup.string().matches(
      /^[0-9]{12}$/,
      "Invalid UAN number. Please enter a valid 12-digit UAN number with only numbers."
    ),
  });
