...
const UploadEmployData = ({ open, handleClose, mutate }: Props) => {
	...
	const handleSubmit = async (values: any) => {
		try {
			...
			Swal.fire(
				`Success!`,
				res?.results?.existUsers?.length
					? `${res?.results?.existUsers?.map(
							(i: string) => `${i} is already exist`
					  )}`
	...
	return (
		...
		>
			...
			<DialogContent className="app-scrollbar" sx={{ p: 3 }}>
				<div className="md:w-[27rem] w-[65vw] md:px-4 px-2 tracking-wide flex flex-col gap-3 text-sm py-2">
					<Formik
