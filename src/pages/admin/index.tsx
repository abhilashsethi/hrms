    ...
    <PanelLayout title="Home - Admin Panel">
      <section className="flex gap-2 py-4">
          ...
          <Grid container spacing={2}>
            {cards?.map((item) => (
              <Grid key={item?.id} item lg={3}>
                <div className="h-56 w-full p-8 flex flex-col rounded-xl shadow-xl justify-between border-b-4 border-theme">
        ...
        <div className="w-1/4 p-2 rounded-xl shadow-xl flex flex-col gap-3">
          {stats.map((item) => (
            <div className="h-40 w-full border-2 rounded-xl py-4 px-6 flex flex-col justify-between tracking-wide">
